...
with tab_score:
    ...
    if "위험등급" in display_df.columns:
        try:
            styled = display_df.style.format(_fmt, na_rep="").map(_risk_color, subset=["위험등급"])
        except AttributeError:
            styled = display_df.style.format(_fmt, na_rep="").applymap(_risk_color, subset=["위험등급"])
