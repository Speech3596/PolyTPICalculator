...
with tab_score:
    ...
    if tpi_view is not None:
        _sc_ordered += ["TPI", "TPI랭크(전체)", "TPI랭크(캠퍼스)", "TPI분위",
                        "위험등급(전체)", "사유(전체)", "위험등급(캠퍼스)", "사유(캠퍼스)"]

    _sc_ordered = [c for c in _sc_ordered if c in display_df.columns]
    display_df = display_df[_sc_ordered]

    # Colour-code 위험등급 columns if present
    def _risk_color(val):
        colours = {
            "Critical-Risk": "background-color:#FF4B4B;color:white;font-weight:bold",
    ...
    _fmt = {c: "{:.2f}" for c in _num_cols}

    _risk_cols = [c for c in ["위험등급(전체)", "위험등급(캠퍼스)"] if c in display_df.columns]
    if _risk_cols:
        try:
            styled = display_df.style.format(_fmt, na_rep="").map(_risk_color, subset=_risk_cols)
        except AttributeError:
            styled = display_df.style.format(_fmt, na_rep="").applymap(_risk_color, subset=_risk_cols)
        st.dataframe(styled, use_container_width=True, height=650)
    else:
        st.dataframe(display_df.style.format(_fmt, na_rep=""), use_container_width=True, height=650)
    ...
    if tpi_view is not None and "위험등급(전체)" in tpi_view.columns:
        with st.expander("위험등급 요약"):
            ...
            _rc_g = (
                ...
                .value_counts()
                .reset_index()
                .rename(columns={"위험등급(전체)": "위험등급(전체)", "count": "학생 수"})
            )
            st.dataframe(_rc_g, use_container_width=True, hide_index=True)
            ...
            _rc_l = (
                ...
                .value_counts()
                .reset_index()
                .rename(columns={"위험등급(캠퍼스)": "위험등급(캠퍼스)", "count": "학생 수"})
            )
            st.dataframe(_rc_l, use_container_width=True, hide_index=True)
...
> **위험등급(전체)** 와 **위험등급(캠퍼스)** 는 **독립적으로** 판정됩니다.
...
| 등급 | 조건 | 설명 |
|------|------|------|
| **Critical-Risk** | 전체 TPI ≤ 하위 20% **AND** B.CV ≤ 하위 20% | 성적도 낮고 과목 간 편차도 큼 → 즉각 개입 필요 |
...
### 위험등급(캠퍼스) — 캠퍼스 내 상대 판정

| 등급 | 조건 | 설명 |
|------|------|------|
| **Local Risk** | 캠퍼스 내 TPI ≤ 하위 20% | 캠퍼스 내 상대적 부진 |
| **Local Top Risk** ⭐ | MAG 레벨 전용: 캠퍼스 내 TPI ≥ 상위 20% AND B.CV ≥ 상위 20% | 캠퍼스 내 최우수 학생 |

**판단 순서 (캠퍼스)**
1. MAG 레벨이면 → **Local Top Risk** 확인
2. **Local Risk** 확인
3. 해당 없음 → 등급 없음(정상)
...
### 예시: 전체 vs 캠퍼스 판정이 독립적인 이유

| 학생 | 캠퍼스 | 위험등급(전체) | 위험등급(캠퍼스) | 설명 |
|------|--------|---------------|-----------------|------|
| A | 판교 | Critical-Risk | Local Risk | 전체에서도, 캠퍼스에서도 하위 |
| B | 판교 | High-Risk | — | 전체 하위 20%이지만, 판교 내에선 하위 20% 아님 |
| C | 분당 | — | Local Risk | 전체에선 정상이지만, 분당 내에선 하위 20% |
| D | 분당 | — | — | 전체·캠퍼스 모두 정상 |

> 캠퍼스별로 전체 기준 Critical/High가 한 명도 없어도, **캠퍼스 기준(Local Risk)은 항상 독립 판정**되므로 캠퍼스 내 하위 20%는 반드시 존재합니다.
""")
    ...
    st.markdown("**위험등급(전체)**")
    _risk_color_global = {
        "Critical-Risk":  ("🔴", "#FF4B4B", "즉각 개입 필요"),
        "High-Risk":      ("🟠", "#FF8C00", "성적 하락 주의"),
        "Latent Risk":    ("🟡", "#FFA500", "과목 편차 과대"),
        "Top Risk":       ("🔵", "#1F4E79", "MAG 최우수 (전체)"),
    }
    for grade, (emoji, color, desc) in _risk_color_global.items():
        st.markdown(
    ...
    st.markdown("**위험등급(캠퍼스)**")
    _risk_color_local = {
        "Local Risk":     ("🟡", "#FFD700", "캠퍼스 내 부진"),
        "Local Top Risk": ("🔵", "#2E86AB", "MAG 최우수 (캠퍼스)"),
    }
    for grade, (emoji, color, desc) in _risk_color_local.items():
        st.markdown(
            f'<span style="background:{color};color:white;padding:2px 10px;'
            f'border-radius:4px;font-weight:bold;">{grade}</span> &nbsp; {emoji} {desc}',
            unsafe_allow_html=True,
